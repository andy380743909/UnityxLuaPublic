    ...
    public class UnityEngineDisplayWrap 
    {
        ...
        static int _m_Activate(RealStatePtr L)
        {
		    try {
                ...
                if(gen_param_count == 4&& LuaTypes.LUA_TNUMBER == LuaAPI.lua_type(L, 2)&& LuaTypes.LUA_TNUMBER == LuaAPI.lua_type(L, 3)&& translator.Assignable<UnityEngine.RefreshRate>(L, 4)) 
                {
                    int _width = LuaAPI.xlua_tointeger(L, 2);
                    int _height = LuaAPI.xlua_tointeger(L, 3);
                    UnityEngine.RefreshRate _refreshRate;translator.Get(L, 4, out _refreshRate);
